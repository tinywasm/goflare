...
> y de la **Fase 1** (`tinywasm/server/httpd`) ya publicadas.
...
> propio `nativeRouter` y **reutiliza `github.com/tinywasm/server/httpd`** (el único
...
2. **Lado nativo: `devserver` reutiliza `server/httpd`, no un router propio.** En coherencia
   con la decisión del maestro ("server/httpd = único implementador nativo"), **borrar**
   `nativeRouter`, `NewRouter` y el `ListenAndServe` propios de `devserver.go`. El
   servidor de desarrollo nativo pasa a construirse sobre `github.com/tinywasm/server/httpd`
   ...
   etc.) se monta como rutas/middleware sobre el `router.Router` de server/httpd, no como un
   adaptador HTTP duplicado.
3. **Lado edge/wasm: `pages` implementa el contrato NUEVO completo.** `wasmContext` y
   `wasmRouter` son el implementador **único** que server/httpd no puede cubrir (server/httpd es
   ...
   - RBAC: el edge aplica `Requires` con su propio autorizador/identidad (equivalente al
     `Identify`/`Authorizer` de server/httpd, con los mecanismos del borde).
...
1. (Prerrequisito) `github.com/tinywasm/router` (Fase 0) y `github.com/tinywasm/server/httpd`
...
3. **Nativo:** borrar `nativeRouter`/`NewRouter`/`ListenAndServe` de `devserver.go`;
   reconstruir el servidor de desarrollo sobre `server/httpd` (estáticos, gzip, cookies, RBAC,
...
5. `go build ./...` nativo (devserver sobre server/httpd) y `GOOS=js GOARCH=wasm go build ./...`
   (el edge `pages`).
...
- **Un solo implementador propio (el edge):** `var _ router.Router = (*wasmRouter)(nil)`,
  ...
  El nativo **ya no es de goflare** — lo aporta `server/httpd`; no debe quedar ningún
...
- **Nativo sobre server/httpd:** un test arranca el `devserver` (que ahora construye un
  `server/httpd.Server`) y sirve estáticos + una ruta registrada por contrato.
- **Doble objetivo:** `devserver` compila nativo (vía server/httpd); `pages` compila

...
- **Static Pages Build:** Delegates static site compilation to `sitec.Build` via a pluggable `SiteBuilder` seam. `sitec` scans project packages for declared producers (`RootCSS()`, `RenderCSS()`, `RenderHTML()`, `IconSvg()`, `Fonts()`, `RenderPages()`), compiles frontend WASM if `web/client.go` exists, and emits all static assets directly to memory before flushing them to disk.

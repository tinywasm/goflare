...
│       └── style.css
├── edge/
│   └── main.go                # //go:build wasm — entrypoint, imports goflare/edge or goflare/workers
...
Files with `//go:build wasm` (everything under `edge/`, `routes/`, `modules/`, `workers/`, `cloudflare/env_wasm.go`) **NEVER** import `fmt`, `strings`, `errors`, `encoding/*`, `net/http`, `log`, `io/ioutil`. Use `tinywasm/fmt`, `tinywasm/json`, `tinywasm/strings`, `tinywasm/fetch` instead.
...
Verification: `grep -rE '^\s*"(fmt|strings|errors|encoding|net/http|log|io/ioutil)"' edge/ routes/ modules/ workers/ cloudflare/env_wasm.go` must return empty.
